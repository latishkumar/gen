...
install: check_prereq ## go install binary info $GOPATH/bin
	packr2 install github.com/latishkumar/gen
...
deps:## analyze project deps
	go list -f '{{ join .Deps  "\n"}}' . |grep "/" | grep -v "github.com/latishkumar/gen"| grep "\." | sort |uniq
...
run_dbmeta: ## generate example project code from sqlite db in ./examples
	go run github.com/latishkumar/gen/_test/dbmeta \
		--sqltype=sqlite3 \
		--connstr "./example/sample.db" \
		--database main
...
gen_readme: set_release ## generate readme file
	go run github.com/latishkumar/gen/readme \
		--sqltype=sqlite3 \
		--connstr "./example/sample.db" \
		--database main \
		--table invoices
